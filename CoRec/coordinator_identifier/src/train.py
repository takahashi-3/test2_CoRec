...
if __name__ == "__main__":
    train_sentences, train_tag, enc_train_tag = process_data(config.TRAINING_FILE)
    dev_sentences, dev_tag, enc_dev_tag = process_data(config.DEVELOPMENT_FILE)

    meta_data = {
        "enc_train_tag": enc_train_tag
    }

    joblib.dump(meta_data, "saved_models/meta_c.bin")
    ...
    for epoch in range(config.EPOCHS):
        ...
        if epoch == 2:
            torch.save(model.state_dict(), "saved_models/model_c_epoch3.bin")
        if epoch == 3:
            torch.save(model.state_dict(), "saved_models/model_c_epoch4.bin")
