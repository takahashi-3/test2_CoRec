...
def main():
    ...
    while (j < len(Lines) - 1):
        ...
        if len(sNums) != 0 and cur_sNum != sNums[0]:
            if (len(token_s) != 0) and (conj_id != -1):
                ...
                try:                    
                    output = predictor.predict(sentence=s)
                    output_string = output.get("trees", "")
                    tree = Tree.fromstring(output_string)
    ...
    print("AllenNLP parser precision:")
    print(p)
    print("AllenNLP parser recall:")
    print(r)
    print("AllenNLP parser f1:")
    print(f1)
    print()
